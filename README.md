...
STCE – Standard Convolution Engine (3×3): For low-level spatial feature extraction.

DWCE – Depthwise Convolution Engine: Channel-wise lightweight convolution for efficient filtering.

PWCE – Pointwise Convolution Engine (1×1): Performs channel mixing and dimensionality adjustment.
...
Precision: FP32 → INT8 via Quantization-Aware Training (QAT)
...
| -------------------- | -------------------------------------------------- |
| **Input**            | 224×224×3 wafer images (RGB)                       |
...
•	The CNN module is configured with the help of a binary (.bin) command file stored on an SD card.
•	This .bin file is a command sequence stream generated by the Lattice Machine Learning Tool.
•	The command code is first written into HyperRAM via AXI before the CNN Accelerator IP core begins execution.
•	During execution, CNN fetches command code from HyperRAM and performs internal operations accordingly.
•	Intermediate feature maps and activation tensors may also be exchanged between CNN and HyperRAM based on command flow.
•	RAW8 pixel input from the csi2_to_parallel interface is downscaled to 224×224 resolution via the crop_downscale_front_224x224 module.
•	This preprocessed 224×224 data is written into HyperRAM through the axi2_hyperbus bridge via axi_ws2m AXI interconnect.
•	Once both command file & image data are valid, the accelerator is triggered at the rising edge of cnn_start.
Image Sensor and Camera Link
The image sensor acts as the direct capture input source via the board’s camera connector.
...
Converted to HEX tensors (224×224×3)
3.	Loading Image to Accelerator RAM
Firmware performs AXI burst writes to CNN RAM
4.	Configuring the CNN Accelerator
Firmware writes: start → layer_id → mode → base addresses of weights/biases → QAT scale/shift parameters
5.	Execution & Polling
...
•	The spi_loader_spram RTL block performs SPI Flash read accesses and streams the content into HyperRAM using AXI.
•	On device power-up, .bit (FPGA image) and .bin (CNN ML command sequence) are auto-loaded to predefined memory ranges:
o	BIT File Address Range: 0x0000000 – 0x00100000
o	Firmware Range: FLASH_START_ADDR – FLASH_END_ADDR
•	AXI logic performs 128-byte × 4-burst write transactions to HyperRAM.
•	axi_ws2m multiplexes multiple AXI write requestors:
o	Slave 0 = spi_loader_spram (highest priority) to guarantee CNN binary availability
•	After each 512-byte segment is written, fetch continues until firmware end address is reached.
User Input and SoC Interface
•	RISC-V machine instructions feed the SoC.
•	Commands are decoded to determine:
Standard pipeline CPU operation
CNN acceleration trigger → DMA image movement → inference start
...
Key AXI Signals
•	Write Address Channel : m_axi_awid, m_axi_awvalid, m_axi_awlen, m_axi_awsize, m_axi_awburst, m_axi_awaddr, m_axi_awprot
•	Write Data Channel : m_axi_wdata, m_axi_wstrb, m_axi_wlast, m_axi_wvalid
•	Write Response Channel : m_axi_bready, s_axi_bvalid, s_axi_bresp, s_axi_bid
•	Read Address Channel : m_axi_arid, m_axi_arvalid, m_axi_arlen, m_axi_arsize, m_axi_araddr, m_axi_arprot
•	Read Data Channel : m_axi_rready, s_axi_rdata, s_axi_rid, s_axi_rresp, s_axi_rvalid, s_axi_rlast

AXI Interface Functions
•	Provides high-speed communication between modules.
•	Manages burst transfers for images, weights, and intermediate feature maps.
•	Prevents bus contention via ready/valid handshake signals.
•	Ensures correct synchronization of data streams during deep CNN pipeline execution.

RISC-V Pipeline Behavior
•	IF: Fetch instructions.
•	ID: Detect CNN accelerator instruction.
•	EX: CPU executes normal instruction or triggers CNN operation.
•	MEM: Performs AXI-based memory transfers.
•	WB: Writes back CPU results; CNN results go to SoC via AXI.
...
Data Movement
•	Images are written to CNN RAM using AXI burst transactions.
•	Accelerator accesses weights, biases, and intermediate activations locally.
Internal Architecture
•	Standard Convolution Engine
•	Depthwise Convolution Engine
•	Pointwise Convolution Engine
...
•	After accelerator completes computation:
...
•	Output wafer features generated inside CertusPro-NX are transmitted to the CYUSB3014 USB3 bridge
•	CYUSB3014 packages & streams data to PC host for visualization and post-classification analytics.


5. FPGA Implementation

Platform: Lattice Semiconductor’s CertusPro-NX Voice and Vision Machine Learning Board
...
The full pipeline, including feature extraction, dense, argmax, and prediction stages, completes in 16,850 cycles at 200 MHz, corresponding to 84.25 µs latency per frame. The accelerator achieves 11.87 kFPS throughput, satisfying real-time inline inspection requirements. It provides a 223.68× speedup over software INT8 inference due to efficient depthwise–pointwise mapping and pipelined execution. Area utilization on CertusPro-NX FPGA is 14.1 mm², and on-chip dynamic power is 2.265 W with energy consumption of 190 µJ per frame, making continuous 24/7 operation feasible. All computation is performed on-chip using BRAM buffers without DDR dependency, preventing bandwidth stalls. RISC-V integration allows direct accelerator control, prediction acknowledgment, and completion without external PC/GPU/cloud involvement, ensuring wafer data security. Deterministic execution with no OS jitter or external memory traffic meets strict fab timing constraints. The architecture supports scaling to larger wafer images or higher channel depth via tile-parallel acceleration, and the modular pipeline allows layer replacement, fusion, or expansion without redesigning memory or control logic. The accelerator sustains inline inspection rate with negligible delay compared to wafer imaging hardware.

🔍 Significance

Real-time defect detection at the fab edge — removing dependency on cloud/HPC.
...
| **Target Hardware**       | Lattice Semiconductor’s CertusPro-NX Voice  Board |
...
W. Jiang et al., “High-Throughput Full-Dataflow MobileNetv2 Accelerator on Edge FPGA,” IEEE TCAD, 2023.

M. Saqlain et al., “Deep CNN for Wafer Defect Identification,” IEEE TSM, 2020.

J. Huang et al., “High-Performance FPGA-Based Depthwise Separable Convolution Accelerator,” Electronics, 2023.

Cuong Pham-Quoc et al., “Efficient FPGA-Based CNN for Edge Computing,” MDPI Electronics, 2022.
